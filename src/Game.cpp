...
bool Game::Init() {
    ...
    if (!window) {
        SDL_ShowSimpleMessageBox(0, "Error creating window: ", SDL_GetError(), window);
        return false;
    }
    ...
    if (!renderer) {
        SDL_ShowSimpleMessageBox(0, "Error creating renderer: ", SDL_GetError(), window);
        return false;
    }
...
void Game::Run() {
    ...
    while (1) {
        ...
        if (curtick - fpstick >= FPS_DELAY) 
        {
            ...
            framecount = 0;
            char buf[100];
            snprintf(buf, 100, "Breakout (fps: %u)", fps);
...
void Game::CheckBoardCollisions() {
    ...
    else if (ball->y + ball->height > board->y + board->height) 
    {
        ...
        {
            SDL_ShowSimpleMessageBox(0, "You lose", "Out of lifes!", window);
            board->CreateLevel();
            SDL_ShowSimpleMessageBox(0, "New game will be started", "Good luck this time!", window);
        }
        ResetPaddle();
    }
